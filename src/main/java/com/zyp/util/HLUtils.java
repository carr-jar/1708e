...
public class HLUtils {
	...
	public static PageInfo<?> getPage(List<?> list,Integer pageNum,Integer currentPage){
		PageInfo<?> pi = new  PageInfo<>(list);
		pi.setPageNum(currentPage);
		pi.setPageSize(pageNum);
		pi.setPrePage(currentPage-1);
		pi.setPages(list.size()%pageNum==0?list.size()/pageNum:list.size()/pageNum+1);
		pi.setNextPage(currentPage+1);
		pi.setHasNextPage(currentPage<list.size());
		pi.setHasPreviousPage(currentPage>1);
